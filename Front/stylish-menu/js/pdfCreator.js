...
function generarCabecera(doc) {
	...
	const variableMotivo = "DA DE BAJA BIENES QUE INDICA";
	const variableFecha = "Santiago, 27 de Octubre de 2021";
	doc
		.image(files.img3.data, 50, 45, { width: 150 })
		.fillColor("#000000")
		.fontSize(10)
...
function generarParrafos(doc) {
	const variableTexto =
		"Que, bajo la actual situación de emergencia sanitaria \
  se ha establecido dentro del Servicio la modalidad de \
  teletrabajo, desarrollándose la mayor parte de las \
  labores posibles de manera remota o digital.";
	...
	if (documento.value == 0) {
		lote = document.getElementById("validationDefault01").value;
		console.log(document.getElementById("validationDefault01"));
	} else {
		lote = "100";
	}
	const variableTexto2 =
  ...
  Activo Fijo bajo la causal Bienes No Sujetos a Venta.";
	...
	doc
		...
		.font("Helvetica")
		.text("1.", 240, 230)
		.text(variableTexto, 260, 230, { width: 240, align: "justify" })
		.text("2.", 240, 305)
		.text(variableTexto2, 260, 305, { width: 240, align: "justify" })
		.fontSize(10)
		.moveDown();
}
